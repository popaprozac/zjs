...
- **Curated subset — 80.2%** (8,122 of 10,130 tests pass after skipping feature families we don't implement yet: BigInt, WeakMap/Set/Ref, FinalizationRegistry, async-iteration, Atomics, SharedArrayBuffer, plus various smaller features). Subset itself is ~14,500 files (~27% of test262), chosen via `scripts/test262/config.json`. The Phase 4.3–4.6 arc grew the denominator by ~2,100 by unmasking private class fields + Proxy + Reflect, so the rate dropped but the **passed-count is up ~1,400** vs the start of that arc. This number answers **"of the parts we claim to support, how spec-correct are we?"**.
