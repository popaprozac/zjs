...
- **Curated subset — 84.3%** (6,741 of 7,997 tests pass after skipping feature families we don't implement: BigInt, Proxy, WeakMap/Set/Ref, FinalizationRegistry, async-iteration, private class fields, Reflect, Atomics, SharedArrayBuffer, optional chaining edge cases). Subset itself is 14,524 files (~27% of test262), chosen via `scripts/test262/config.json`. This number answers **"of the parts we claim to support, how spec-correct are we?"**.
- **Full suite — 37.3%** (17,303 of 46,364 tests pass against `test/language/` + `test/built-ins/` with no feature-skip list, matching the methodology of dashboards like [test262.fyi](https://test262.fyi)). This number answers **"across the entire spec surface, how complete is the engine?"** — missing-feature failures are real failures here. Run via `make test262-full`. For context: QuickJS NG reports 82.4% on the same methodology; the ~45-point gap is feature breadth (BigInt, Proxy, WeakRef, private fields, async iteration), not bugs in features we do ship.

Both numbers are useful; quoting only one out of context is misleading. Live dashboard for the curated subset at `docs/conformance/index.html` (macOS) — Windows results at `docs/conformance/index-windows.html`.
