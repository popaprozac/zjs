...
**Conformance:** 84.3% of the test262 *curated subset* — 6,741 of 7,997 tests pass after skipping feature families we don't implement yet (BigInt, Proxy, WeakRef, async-iteration, private class fields, Reflect, Atomics, SharedArrayBuffer). The subset itself is 14,524 files (~27% of test262), chosen via `scripts/test262/config.json`. This number measures **how spec-correct we are on the parts we claim to support** — it's not directly comparable to dashboards like [test262.fyi](https://test262.fyi) that run the full ~53k-file suite and count missing-feature skips as failures. `make test262-full` reports that absolute-methodology number alongside, for honest framing. Live dashboard at `docs/conformance/index.html` (macOS) — Windows results at `docs/conformance/index-windows.html`.
