...
| `fetch` | WinHTTP async | ✅ shapes + `data:` URLs + **live HTTP transport** (wintercg `fetch` area: loopback HTTP server via `scripts/wintercg/http_echo_server.py` — GET text/json, status codes, response headers, POST body + request headers). Verified 17/17 on Windows (WinHTTP). | live **TLS**/redirect/chunked-streaming still unprobed (no cert infra in CI); the plaintext request/response/header/body path is now covered |
